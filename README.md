...
3. 左の「⚙ プロジェクトの設定」→「スクリプト プロパティ」で必要なものを追加する。
   ...
   | `APPROVER_TOKEN` | 任意 | 失敗談の承認に必要な合言葉。上級生だけに渡す |
...
うまくいかなかった話ほど後輩の役に立つのに、いちばん残りません。「🧯 失敗談を書く」タブは、
それを**あとから機械的に追える形**で受け取るための入力欄です。

入力欄はそのまま意思決定の鎖になっています。

| 欄 | 意図 |
| --- | --- |
| 問い / 検討した候補 / 実際に試したこと | 「何を決めようとして、何を試したか」を分解して残す |
| 起きた失敗・症状（必須） | ここだけは必ず埋める。他は空欄でよい |
| 失敗の理由 ＋ **確からしさ**（実際に確認した / 推測 / 分からない） | 事実と推測を、入力の時点で分けて持つ |
| そのあとの判断 / 結果 / 次にやる人へ | 判断とその後を、失敗と同じ記録に閉じ込める |
| 対象（Wet / Dry）・想定読者（B3 / B4 / M1） | 読む人に合わせて出し分けるための軸 |
| 出典（Slack投稿・Boxのファイル・録音・実験ノート） | **原文に戻れること**がこの仕組みの生命線 |
| 関連する記録 | 同じ意思決定の続きなら選ぶ。`decisionId` で束ねて追える |

設計上の約束（将来Slack・Boxを横断して復元するための土台）:

- **書かれなかった項目は捨てず、資料に「不明（未記録）」と明記する。** 推測で埋めない。
- **資料本文に入るのは入力された事実だけ。** AIの補足は本文に混ぜず、末尾に「🤖 AIによる補足（推測を含む）」として区別する。
- **承認されるまで教材にしない。** 投稿は「承認待ち」で保存され、上級生が承認するまで承認済みの一覧に出ない。
  スクリプトプロパティ（またはサーバー版の環境変数）に `APPROVER_TOKEN` を設定すると、承認に合言葉が要る。
- 資料箱では「🧯 失敗談 / 📄 引継ぎ資料」「✅ 承認済み / 📝 承認待ち」で絞り込める。
